...
Node Parser::var_list() {
	Node var_list_node("<varList>");

	// Expect an identifier
	if (current_token.id == IDENT_TK) {
		var_list_node.add_child(Node(current_token.instance));
...
Node Parser::block() {
	...
	if (current_token.id == KW_TK && current_token.instance == "start") {
		...
		block_node.add_child(Node("start"));
	} else {
	...
	if (current_token.id == KW_TK && current_token.instance == "stop") {
		...
		block_node.add_child(Node("stop"));
	} else {
...
Node Parser::read() {
	...
	// Expect an identifier
	if (current_token.id == IDENT_TK) {
		read_node.add_child(Node(current_token.instance));
		match(IDENT_TK); //consume identifier
	} else {
...
Node Parser::iter() {
	...
	if (current_token.id == KW_TK && current_token.instance == "iterate") {
		...
		iter_node.add_child(Node("iterate"));
	} else {
...
Node Parser::assign() {
	...
	if (current_token.id == KW_TK && current_token.instance == "set") {
		...
		assign_node.add_child(Node("set"));
	} else {
	...
	// Expect an identifier
	if (current_token.id == IDENT_TK) {
		assign_node.add_child(Node(current_token.instance));
		match(IDENT_TK); //consume identifier
	} else {
...
Node Parser::exp() {
	...
	while (current_token.id == OP_TK && (current_token.instance == "+" || current_token.instance == "-")) {	
		...
		exp_node.add_child(Node(operator_instance)); 
...
Node Parser::m() {
	...
	while (current_token.id == OP_TK && current_token.instance == "%") {
		m_node.add_child(Node("%")); // Add the operator %
...
Node Parser::n() {
	...
	if (current_token.id == OP_TK && current_token.instance == "-") {
		n_node.add_child(Node("-")); // Add the operator '-'
		match(OP_TK); // Consume the '-' operator
...
Node Parser::r() {
	...
	if (current_token.id == OP_TK && current_token.instance == "(") {
		match(OP_TK); // Consume the '(' operator
		r_node.add_child(Node("(")); // Add the '(' operator
	...
	// Check for an identifier
		...
		r_node.add_child(Node(current_token.instance)); // Add the identifier
		match(IDENT_TK); // Consume the identifier
	}

	// Check for an integer
	else if (current_token.id == NUM_TK) {
		r_node.add_child(Node(current_token.instance)); // Add the integer
