...
void Generator::generate() {
        // Generate the code by traversing the parse tree
        traverse(tree.get_root()); // Start from the root node
...
void Generator::handle_var_list(const Node& node) {
    ...
    if (children.size() == 4 && children.at(3).get_data() == ";") {
        ...
        const string& var_name = children.at(0).get_data();  // Extract the variable name
        allocate_storage(var_name);  // Add to declared variables
        // Recursive on the next part of <varList>
...
void Generator::handle_print(const Node& node) {
    ...
    if (!children.empty()) {
        ...
        if (!value.empty() && isalpha(value.at(0))) {
            code << "WRITE " << value << "\n"; // Write the value from the identifier
        } else { // Literal value
...
void Generator::handle_stat(const Node& node) {
    ...
    if (child_label == "<read>") { handle_read(children.at(0)); } 
    else if (child_label == "<print>") { handle_print(children.at(0)); }
    else if (child_label == "<block>") { handle_block(children.at(0)); } 
    else if (child_label == "<cond>") { handle_cond(children.at(0)); } 
    else if (child_label == "<iter>") { handle_iter(children.at(0)); } 
    else if (child_label == "<assign>") { handle_assign(children.at(0)); } 
    else {
...
void Generator::handle_read(const Node& node) {
    const string& var_name = node.get_children().at(1).get_data(); // Extract the variable name
    code << "READ " << var_name << "\n";
}
...
void Generator::handle_cond(const Node& node) {
    ...
    code << label << ": NOOP\n";
    traverse(children.at(6));  // Traverse <stat>
...
void Generator::handle_relational(const Node& node) {
    ...
    if (rel_op == ".ge.") { code << "BRNEG "; } 
    else if (rel_op == ".le.") { code << "BRPOS "; } 
    else if (rel_op == ".gt.") { code << "BRZNEG "; } 
    else if (rel_op == ".lt.") { code << "BRZPOS "; } 
    else if (rel_op == "**") { code << "BRZERO "; } 
    else if (rel_op == "~") {
        // Branch if NOT zero (inverse of BRZERO)
        string false_label = create_label();  // Label for the "false" block
        code << "BRZERO " << false_label << "\n";  // Skip true block if zero
    } else {
        cerr << "Error: Unknown relational operator '" << rel_op << "'!\n";
    }
}
...
void Generator::handle_exp(const Node& node) {
    ...
    if (children.size() > 3) {
        ...
        if (op == "+" || op == "-") {
            ...
            if (op == "+") { code << "ADD " << right_temp << "\n"; } 
            else if (op == "-") { code << "SUB " << right_temp << "\n"; }
        } else {
            // Treat the entire node as an expression
            for (size_t i = 0; i < children.size(); i++) {
                handle_m(children.at(i));
                if (i < children.size() - 1) {
                    const string& sub_op = children.at(i + 1).get_data();
                    string temp = create_temp();
                    code << "STORE " << temp << "\n";
                    if (sub_op == "%") { code << "MULT " << temp << "\n"; } 
                    else if (sub_op == "/") { code << "DIV " << temp << "\n"; }
                }
            }
        }
    } else if (children.size() == 3) {
        // Process binary operations
        handle_m(children.at(1)); // Right-hand operand
        string right_temp = create_temp();
        code << "STORE " << right_temp << "\n";
        ...
        if (operator_token == "+") { code << "ADD " << right_temp << "\n"; } 
        else if (operator_token == "-") { code << "SUB " << right_temp << "\n"; }
    } 
    else if (children.size() == 1) {     
        handle_m(children.at(0)); // Process a single operand
    } else {
...
void Generator::handle_n(const Node& node) {
    ...
    if (children.size() == 1) {
        ...
        handle_r(children[0]); // Left operand
        std::string left_temp = create_temp();
        code << "STORE " << left_temp << "\n";

        handle_n(children[2]); // Right operand
        ...
        // Handle other cases (negation, complex expressions, etc.)
        std::cerr << "Error: Unsupported structure in <N> node.\n";
    }
